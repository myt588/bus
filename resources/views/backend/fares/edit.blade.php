...
@section('breadcrumb')
    ...
    <li><a href="/admin/fares/%{{$fare->id}}/edit">Edit</a></li>
    ...
    <section class="content">
        <div class="box box-warning"> 
            <div class="box-body">
                ...
                @for($i = 0; $i < count($trips_selected); $i ++)
                <div class="form-group {{ $errors->has('trip_id') ? 'has-error' : ''}}">
                    <label class="col-sm-3 control-label">{{ $i == 0 ? 'Trips:' : '' }}</label>
                    <div class="col-sm-6">
                        {!! Form::select('trip_id[]', $trips, $trips_selected[$i], ['class' => 'form-control']) !!}
                        {!! $errors->first('trip_id[]', '<p class="help-block">:message</p>') !!}
                    </div>
                    <div class="col-sm-1">
                        <button type="button" class="btn btn-default {{ $i == 0 ? 'addButton' : 'removeButton' }}">
                        <i class="{{ $i == 0 ? 'fa fa-plus' : 'fa fa-minus' }}"></i></button>
                    </div>
                </div>
                @endfor
